...
GATE_NAME = "destructive_batch_v1"
...
REPORT_JSON = "reports/destructive_batch_v1_results.json"
REPORT_MD = "reports/destructive_batch_v1_summary.md"
...
def main():
        ...
        for t in TEMPLATES:
            ...
            for bsize in BATCH_SIZES:
                ...
                for i in range(ITERATIONS_PER_CONFIG):
                    # Pick a non-overlapping slice if possible, else wrap around
                    ...
                    try:
                        t0 = time.time()
                        # STRICTION: no fallback, no retry
                        res = client.chat(
                        ...
                        # In runtime_adapter result.model is the actual model name used
                        if res.model != model:
                        ...
                        if passed:
                            ...
                            if res.usage:
                                ...
                                tokens.append(prompt_tokens + completion_tokens)
                        else:
                            fail_reason = reason
                            break # Fail iteration -> fail config
                            
                    except Exception as e:
                        fail_reason = f"Exception: {str(e)[:50]}"
                        break
                
                if config_success_count == ITERATIONS_PER_CONFIG and not abort_model:
                    print("✅ PASS")
                    ...
                    abort_model = True # Stop escalation for this model
                    
            model_results["templates"][t] = {
                "results": template_results,
                "max_pass": template_max_pass
            }
            if template_max_pass > 0:
                overall_max_pass = max(overall_max_pass, template_max_pass) # This is not strictly right per template, but global max
            
        # Overall max pass is the lowest max_pass across all templates for that model to be "overall stable"
        min_max = 20
        all_passed = True
        for t in TEMPLATES:
            tm = model_results["templates"][t]["max_pass"]
            if tm < min_max: min_max = tm
            if tm == 0: all_passed = False
            
        model_results["max_pass_batch_size_overall"] = min_max if all_passed or min_max > 0 else 0
    ...
    md_lines = [
        ...
        "\n| Model | Overall Max Batch | Temp A (Len) | Temp B (JSON) | Temp C (PH) | Status |",
        "|---|---|---|---|---|---|"
    ]
    
    for model, mdata in results.items():
        max_o = mdata["max_pass_batch_size_overall"]
        max_a = mdata["templates"]["A"]["max_pass"]
        max_b = mdata["templates"]["B"]["max_pass"]
        max_c = mdata["templates"]["C"]["max_pass"]
        status = "✅ PASS" if max_o >= 10 else "❌ FAIL"
        md_lines.append(f"| {model} | **{max_o}** | {max_a} | {max_b} | {max_c} | {status} |")
